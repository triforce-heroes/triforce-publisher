...
describe("QueryGenerator", () => {
  ...
  const tests: Test[] = [
    ...
    [
      ...
      "INSERT INTO `projectEntries` (`projectId`, `resource`, `reference`, `sources`, `metadata`, `updatedAt`) VALUES (1, NULL, 123, '{\"hello\":[\"en\"]}', '{\"level\":5}', 0) ON CONFLICT (`projectId`, `resource`, `reference`) DO UPDATE SET `sources` = `excluded`.`sources`, `updatedAt` = `excluded`.`updatedAt`, `translation` = (CASE WHEN `sources` != `excluded`.`sources` THEN NULL ELSE `translation` END), `translationBy` = (CASE WHEN `sources` != `excluded`.`sources` THEN NULL ELSE `translationBy` END), `translationAt` = (CASE WHEN `sources` != `excluded`.`sources` THEN NULL ELSE `translationAt` END), `metadata` = (CASE WHEN (`excluded`.`metadata` IS NULL AND `sources` != `excluded`.`sources`) THEN NULL WHEN `excluded`.`metadata` IS NULL THEN `metadata` ELSE JSON_PATCH(COALESCE(`metadata`, '{}'), `excluded`.`metadata`) END)",
    ],
    [
